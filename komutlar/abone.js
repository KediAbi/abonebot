...
exports.run = async (client, message, args) => {
  ...
  member.roles.add("911231838931005441"); //abone rolü idsi
  let aboneembed = new Discord.MessageEmbed()
    .setTitle("・Abone Rolü Verildi")
    .setColor("#f0eb0e")
    .setDescription(
      `${message.author} **:** Abone Rolü Verme Sayısı: **${
        kayıtsayı ? `${kayıtsayı}` : "0"
      }**`
    )
  ...
  let abonelog = client.channels.cache.get("1006132434158506035"); // abone log id
  if (!abonelog)
    return message.channel.send("Bu kanalda abone rolü verlilemez!");
